...
ssp_results_enhanced <- list.files(here("data", "2_study1"), pattern = "^ssp_fit_enhanced", full.names = TRUE) %>%
  map_df(function(x) read_csv(x)) %>%
  rename(id = subject) %>%
  rename_with(.cols = !matches("id"), ~str_replace_all(.x, ., str_c(., "_flanker_enh")))
 
ssp_results_degraded <- list.files(here("data", "2_study1"), pattern = "^ssp_fit_degraded", full.names = TRUE) %>%
  map_df(function(x) read_csv(x)) %>%
  rename(id = subject) %>%
  rename_with(.cols = !matches("id"), ~str_replace_all(.x, ., str_c(., "_flanker_deg")))
